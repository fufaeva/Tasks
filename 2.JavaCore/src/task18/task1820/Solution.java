...
public class Solution {
    public static void main(String[] args) throws IOException {
        ...
        while (inputStream.available() > 0) {
            sb.append(Character.toChars(inputStream.read()));

        }

        String[] numbersForOutput = sb.toString().split(" ");
        for (int i = 0; i < numbersForOutput.length; i++) {
            int num = Math.round(Float.parseFloat(numbersForOutput[i]));
            fileOutputStream.write(Integer.toString(num).getBytes());
